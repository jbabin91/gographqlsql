...
import (
	...
	"github.com/jbabin91/gographqlsql/gql"
	"github.com/jbabin91/gographqlsql/db"
	"github.com/jbabin91/gographqlsql/server"
)
...
func initializeAPI() (*chi.Mux, *postgres.Db) {
	...
	db, err := postgres.New(
		postgres.ConnString("localhost", 54320, "docker", "docker"),
	)
	if err != nil {
		log.Fatal(err)
	}
